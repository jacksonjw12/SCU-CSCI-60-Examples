...
// If we want to use Bag in a range-for-loop, then we need to add two member
// functions to our class: begin and end. The begin function returns a pointer
// to the first element in the Bag and the end function returns a point to one
// past the last element.
template <typename T>
typename Bag<T>::value_type* Bag<T>::begin() {
  return data_;  // The address of data_[0]
}

template <typename T>
typename Bag<T>::value_type* Bag<T>::end() {
  return data_ + size_;  // The address of data_[size_]
}
